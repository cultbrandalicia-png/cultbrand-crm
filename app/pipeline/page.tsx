              ...
              {prospectos
                ...
                .map((prospecto) => (
                  <div key={prospecto.id} className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 cursor-pointer hover:shadow-md transition-shadow">
                    <h3 className="font-medium text-sm">{prospecto.nombre_empresa}</h3>
                    {prospecto.persona_contacto && (
                      <p className="text-xs text-gray-500 mt-1">{prospecto.persona_contacto}</p>
                    )}
                    {prospecto.sector && (
